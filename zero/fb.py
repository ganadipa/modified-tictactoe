...
def ai_move(state, symbol, round, depth = False):
                ...
                if round < 5:
                    # if ((row-1,col-1) in nearby(latest_move[symbol == o])):
                        ...
                        if depth is False:
                            ...
                            score = minimax(state, not(isMaximize), depth)
...
def update_data(numround):
    ...
    with open('data_3.csv', 'r') as f:
        ...
        for string_state, eval, bmove in reader:
            lst.append([string_state, eval, bmove])
            count -= 1
    
    for i in range(-1,count,-1):
        print('in row', i)
        string_state, eval, bmove = lst[i]
        if bmove == '(-1,-1)':
